...
export default function Pricing() {
  ...
  function handleSubscriptionDay(){
    setSubscribeDetail({...subscribeDetail, isSubscribe:true, plan:15,isLogin:true})
    
  }
  function handleSubscriptionMonth(){
    setSubscribeDetail({...subscribeDetail, isSubscribe:true , plan:30, isLogin:true})
  }
  ...
  return (
    ...
    <p>Day Pass</p>
    <p><sup style={{fontSize:"1.5em"}}>$</sup> <span className={PricingStyle.price}><sub>20</sub></span><br/><span>/pass</span></p>
    ...
    <ul >
      ...
      <button className={PricingStyle.btn}  onClick={()=>{ let a=window.confirm("you are getting Subscribed to 15 days plan") ;if(a){handleSubscriptionDay(); routeChange()} else{route()}}}>{subscribeDetail.isSubscribe && subscribeDetail.plan===15 ?"Get Started":"Subscribe"}</button>
    </div>
    <div className={PricingStyle.Month}>
    <p> Month to Month </p>
    <p><sup style={{fontSize:"1.5em"}}>$</sup><span className={PricingStyle.price}><sub>90 </sub></span><br/><span>/month</span></p>
    ...
    <ul>
      ...
      <button className={PricingStyle.btn}  onClick={()=>{ let a=window.confirm("you are getting Subscribed to a monthly plan");if(a){ handleSubscriptionMonth();routeChange()}else{route()}}}>{subscribeDetail.isSubscribe && subscribeDetail.plan===30?"Get Started":"Subscribe"}</button>
      </div>
